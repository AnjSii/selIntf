...
@Service
@Transactional
public class GoodsIrregularitiesServiceImpl implements GoodsIrregularitiesService {
	...
	@Override
	public List<GoodsVO> getGoodsIrregularities(String userName) throws NullPointerException, IllegalArgumentException,
			...
			case 2:
				...
				List<Goods> goodsList = this.goodsService
						.query("select obj from Goods obj where obj.goods_status = :goods_status and " +
										"obj.goods_store.id = :store_id and obj.deleteStatus = :delete_status " +
										"order by obj.goods_seller_time desc",
								params, -1, -1);
	...
	@Override
	public void goodsAppeal(Long goodsId, AppealReasnRequestVO appealReasnRequestVO, String userName) throws NullPointerException, IllegalArgumentException, EntityNotFoundException, ViolationException {
		if (userName == null || appealReasnRequestVO == null) {
			throw new NullPointerException();
		}
		if (userName.isEmpty() || appealReasnRequestVO.getAppealReasn().isEmpty()) {
			throw new IllegalArgumentException();
		}
		...
		if (goods.getGoods_status() != -2) {
			throw new ViolationException("当前商品不是违规商品");
		}
		int store_status = store.getStore_status();
		switch (store_status) {
			case 1:
				throw new ViolationException("您的店铺正在审核中");
			case 2:
				goods.setCancel_violation_apply(appealReasnRequestVO.getAppealReasn());
